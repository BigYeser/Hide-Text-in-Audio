...
%          fileexchange/37847-data-encryption-standard--des-/content/DES.m
% 1.0 Parity Check. 
...
%% 2.1 Define permutation PC1(C & D).
...
for k=1:16
    C=circshift(C_0,-sum(Left_Shift(1:k)),2);
    D=circshift(D_0,-sum(Left_Shift(1:k)),2);
    CD=[C D];
    K{k}=CD(PC2);
end
...
%% 4.2 Initial values for Feistel Network. Pre-alocate space.
switch Mode
    case 'ENC'
        PT_0=dec2bin(str,8)';
        PT_1=logical(PT_0=='1');
...
%% 5.1 Main Loop. Outer for loop runs DES on all 64-bit blocks.
...
% Inner for loop computes Feistel values for each block, 16 rounds    
    ...
    switch Mode
        case 'ENC'
            DESOUT{i}=binaryVectorToHex(DESbinary);
        case 'DEC'
